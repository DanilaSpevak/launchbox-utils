...
`docs/plans/<work_item_id>-audit-refresh-<N>.md`, который ссылается на прежний PR,
...
`claimed_by`, scope, `depends_on`, cumulative `review_history`, `defect_history`
и `review_round`, но очищаются `candidate_sha`, `accepted_code_sha` и текущие
design/verdict fields; активные `auditor`, `recorder` и reviewer становятся
`null`. Старый PR
...
`review_history`, `defect_history` и `review_round` являются append-only и
cumulative для всего `work_item_id`, включая все superseded PR и refresh/repeat
ветки; их нельзя очищать или обнулять. Новый reviewer не может совпадать ни с
одним historical reviewer этого work item. Каждое blocking finding получает
стабильный `defect_id`: SHA-256 canonical UTF-8 JSON первого появления с
фиксированными ключами `invariant`, `surface`, `minimal_repro`, опубликованного в
verdict. `defect_history` добавляет для каждого появления `defect_id`, текущую
classification, global `round`, `candidate_sha`, `verdict_id` и resolution.
Reclassification не создаёт новый ID. Семантически тот же failure повторно
использует прежний `defect_id`; спор о соответствии является
`decision_required`. Поэтому лимит пяти rounds и повтор после двух fixes
считаются по всей superseded-цепочке, а не по одному PR или baseline.
...
Каждый job обязан явно checkout'ить `github.event.pull_request.head.sha`,
сравнить его с `git rev-parse HEAD` и опубликовать проверенный SHA в job summary;
orchestrator сверяет это evidence с `candidate_sha`.
...
Для implementation work item, если baseline продвинулся, verdict аннулируется,
задача возвращается
...
`work_item_id`, `claimed_by`, cumulative `review_history`, `defect_history` и
global `review_round`, но заменяет `base_sha`/`branch`, очищает
...
и устанавливает `status: preparing`.
...
Для audit work item drift из `accepted` либо `awaiting_merge` использует ту же
`agent/<work_item_id>-audit-refresh-<N>` семью, что repeat после remediation, но
начинает новый PR в `preparing`, а не `suspended`. Новый
`docs/plans/<work_item_id>-audit-refresh-<N>.md` фиксирует новый canonical
`base_sha`, прежний immutable scope и причину повтора. State transfer сохраняет
`work_item_id`, `claimed_by`, scope, `depends_on`, cumulative `review_history`,
`defect_history` и global `review_round`, очищает candidate,
acceptance/design/current-verdict fields и активные audit/reviewer roles.
Orchestrator назначает нового auditor и выполняет
`preparing → auditing → recording → reviewing`; старый verdict и путь через
`ready`/`implementing` запрещены. Старый PR закрывается как superseded только
после проверенного переноса authority; частичный перенос даёт
`decision_required`.
