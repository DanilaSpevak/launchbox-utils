...
| `accepted` | `awaiting_merge`, `implementing` или `decision_required` |
| `awaiting_merge` | `integrated`, `implementing` или `decision_required` |
...
из `accepted` либо `awaiting_merge` в `implementing`, новый default HEAD
...
опубликованной branch. Implementer переносит task commits в replacement branch.
...
`candidate_sha`, `accepted_code_sha`, `verdict_id`, `verdict_sha256`,
`review_history`, устанавливает `review_round: 0` и `status: implementing`.
