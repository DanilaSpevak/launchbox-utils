...
- Линейное извлечение XML declaration из source prefix размером не более 1 MiB;
  более длинная declaration отклоняется fail-closed с bounded диагностикой.
...
- Namespace semantics не зависят от глобального `ET.register_namespace()`:
  ...
  URI.
- Namespace declarations с пустым или зарезервированным prefix, а также QNames
  с несвязанным либо malformed prefix отклоняются parser'ом.
- Извлечение declaration выполняется линейно в пределах 1 MiB и окружено
  cancellation checkpoints; превышение bound не приводит к потере declaration
  или неограниченному post-parse scan.
...
| Declaration длиннее 1 MiB | Bounded `ParseError` до plan/backup/stage; regex scan отсутствует |
...
| Пустой namespace prefix `xmlns:` | Parse завершается fail-closed как malformed XML |
...
Приёмка повторно выполнена 2026-07-16 после независимого post-acceptance review
и шестого review/fix цикла:

- codec/end-to-end matrix: 18 тестов успешно;
- read/write и обе mutation operations: 107 тестов успешно;
- полный discovery: 250 тестов успешно;
...
- отдельные regressions подтверждают линейное сохранение declaration внутри
  1 MiB bound, bounded fail-closed отказ за его пределами и отклонение пустого
  `xmlns:` prefix; дополнительная дифференциальная namespace/QName-матрица из
  672 случаев совпала со стандартным parser'ом;
