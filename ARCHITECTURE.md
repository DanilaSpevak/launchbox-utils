...
Path replacement maintains its canonical planned file states incrementally during scan, so repeated file errors remain O(1). Per-change states are synchronized from that index during the existing terminal result or manifest traversal, without an extra full-plan pass after cancellation. A cancelled apply still writes one complete, atomic manifest during protected finalization. That required manifest serialization is intentionally allowed to take O(N) time in the number of changes; incomplete manifests and background finalization are not used.
