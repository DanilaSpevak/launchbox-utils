...
Only repeated canonical variants under the same immediate XML parent inside one logical component are automatic duplicates. The parent object is part of the contextual signature: otherwise identical records under different parents are preserved and diagnosed as `#parent`. An entry whose own `AdditionalApplication.tail` contains significant parent mixed content is never removable and participates in `#parent-content` ambiguity regardless of whether it is the first representative or a later occurrence; a later safe duplicate may still be removed. If a component contains multiple canonical variants, one representative of each is preserved and the component is reported as ambiguous for manual review; a root-tag or namespace difference is diagnosed as `#root`. For `A, A, B`, only the second `A` is removable.
